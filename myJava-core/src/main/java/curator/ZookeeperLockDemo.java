...
//        }
    ...
    @Test
    public static void testSharedLock1(){
     ...
     * @throws Exception
     */
    private static void sharedLock1() {
        CuratorFramework client = getClient();
        //这两个都是共享锁
        // new InterProcessMutex(client, path)
        // new InterProcessSemaphoreMutex(client, path)

        InterProcessSemaphoreV2 sharedLock = new InterProcessSemaphoreV2(client, "/sharedlock", 3);
        Lease acquire = null;
        Collection<Lease> leases = null;
        try {
            //锁是否被获取到
            //超时 不在进行操作
            //if(sharedLock.acquire(5000, TimeUnit.MILLISECONDS)){
            //sharedLock.acquire();
            System.out.println(Thread.currentThread().getName() + "  is  ready get the shared lock");
            leases = sharedLock.acquire(1);
            System.out.println(Thread.currentThread().getName() + "  is  get the shared lock"+System.currentTimeMillis());
            Thread.sleep(5000);
    ...
    private static void sharedLock(){
        ...
        InterProcessSemaphoreMutex sharedLock = new InterProcessSemaphoreMutex(client, "/sharedlock");
        try {
            //锁是否被获取到
            //超时 不在进行操作
            if(sharedLock.acquire(5000, TimeUnit.MILLISECONDS)){
